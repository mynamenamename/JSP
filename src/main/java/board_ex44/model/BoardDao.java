	...
	*/
	public int insert( BoardVO rec ) throws BoardException
	{
		...
		try{
			...
			//* sql 문장 만들기
			String putQuery		= "INSERT INTO board_ex22 (seq,title,writer,content,regdate,cnt,pass) VALUES (seq_board.nextval, ?,?,?,sysdate,0,?) ";  
	...
	*/
		...
		try{
			...
			con	= DriverManager.getConnection( dbUrl, dbUser, dbPass );
			 System.out.println("DB 연결 성공"); 
				...
				// * sql 문장만들기
				String sql= "SELECT seq, title, writer, regdate, cnt FROM board_ex22";
				// * 전송객체 얻어오기
	...
	public BoardVO selectById(int seq) throws BoardException
	{
		...
		try{
			...
			con	= DriverManager.getConnection( dbUrl, dbUser, dbPass );
			// * sql 문장만들기
			String sql="SELECT * FROM board_ex22 WHERE seq=? ";
			// * 전송객체 얻어오기
			   ps=con.prepareStatement(sql);
			   ps.setInt(1, seq);
			// * 전송하기
			...
			// * 결과 받아 BoardVO변수 rec에 지정하기
			   if(rs.next()) {
               	...
               	rec.setSeq(rs.getInt("SEQ")); //화면에 찍히지 않아도  pk니께  꼭 가져오기
	...
	public void increaseReadCount( int seq ) throws BoardException
	{
		...
		try{
			...
			con	= DriverManager.getConnection( dbUrl, dbUser, dbPass );
			   ...
			   String sql = "UPDATE board_ex22 SET cnt=cnt+1 WHERE seq=?";
				// * 전송객체 얻어오기
				ps		= con.prepareStatement( sql );
				ps.setInt(1, seq);
